...
# Set PATHs
# PATH_TO_SENTEVAL = '/home/omsus20/PycharmProjects/TaDSE/senteval/'
...
class CLTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, num_items_in_batch, return_outputs=False):
        ...
        if (self.state.global_step + 1) % self.args.logging_steps == 0:
            log_dict = {}
            if outputs.simcse_utt_loss is not None:
                log_dict["simcse_utt_loss"] = outputs.simcse_utt_loss.item()
            if outputs.simcse_ptn_loss is not None:
                log_dict["simcse_ptn_loss"] = outputs.simcse_ptn_loss.item()
            if outputs.mlm_loss is not None:
                log_dict["mlm_loss"] = outputs.mlm_loss.item()
            if outputs.pairwise_loss is not None:
                log_dict["pairwise_loss"] = outputs.pairwise_loss.item()
            self.log(log_dict)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, 0, model, trial, epoch, None, start_time)
